import Image from "next/image";
import { Users, Play } from "lucide-react";

export default function GroupClasses() {
    return (
        <section id="group-classes" className="py-20 px-6 bg-[#fdfbf9]">
            <div className="max-w-7xl mx-auto">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">

                    {/* COLUMNA IZQUIERDA: TEXTO */}
                    <div className="lg:col-span-4 space-y-6">
                        <div className="inline-flex items-center justify-center p-2 rounded-full bg-[#d7bdb3]/10 text-[#d7bdb3]">
                            <Users size={32} strokeWidth={1.5} />
                        </div>

                        <h2 className="text-4xl md:text-5xl font-serif text-neutral-800 leading-tight">
                            Group classes
                        </h2>

                        <div className="space-y-4 text-neutral-600 text-lg leading-relaxed max-w-sm">
                            <p>
                                More than just a space for movement, it’s a ritual of connection. Each practice is guided with presence, deep listening, and truth.
                            </p>
                            <p className="relative inline-block">
                                You don’t need any experience, just your willingness to breathe, feel, and come back to yourself. Because when the body awakens, the soul does too. And that’s where the magic begins.
                                {/* El subrayado decorativo sutil de la imagen */}
                                <span className="absolute bottom-1 left-0 w-full h-[6px] bg-[#d7bdb3]/20 -z-10"></span>
                            </p>
                        </div>

                        <button className="mt-4 bg-[#8e735b] text-white px-8 py-3 rounded-full font-medium hover:bg-[#7a624e] transition-all transform hover:scale-105 shadow-md">
                            View schedule
                        </button>
                    </div>

                    {/* COLUMNA DERECHA: VIDEOS/IMÁGENES */}
                    <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">

                        {/* Tarjeta 1 */}
                        <div className="relative group overflow-hidden rounded-3xl aspect-[4/5] shadow-xl">
                            <video
                                src="/videos/group1.mov" // Reemplaza con tu segunda ruta
                                autoPlay
                                loop
                                muted
                                playsInline
                                className="absolute inset-0 w-full h-full object-cover transition-transform duration-700"
                            />
                            <div className="absolute inset-0 bg-black/10 group:bg-black/20 transition-colors" />
                        </div>

                        {/* Tarjeta 2 */}
                        <div className="relative group overflow-hidden rounded-3xl aspect-[4/5] shadow-xl">
                            <Image
                                src="/images/group-yoga-2.jpg" // Reemplaza con tu ruta
                                alt="Meditation Group"
                                fill
                                className="object-cover transition-transform duration-700 group:scale-110"
                            />
                            <div className="absolute inset-0 bg-black/10 group:bg-black/20 transition-colors" />
                        </div>

                    </div>
                </div>
            </div>
        </section>
    );
}